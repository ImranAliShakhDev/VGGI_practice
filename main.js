...
function CreateSurfaceData() {
    ...
    for (let u = 0; u < 360; u += step) {
        for (let v = 0; v < 360; v += step) {
            ...
            textureList.push(unext / 360, v / 360);
        }
    }
...
function CalcXYZ(u, v) {
    ...
    return [0.5 * (CalcParData * (1 + Math.cos(deg2rad(u))) + C * ((1 - Math.cos(deg2rad(u))) / CalcParData)) * Math.cos(deg2rad(v)), 0.5 * (f_v * (1 + Math.cos(deg2rad(u))) + C * ((1 - Math.cos(deg2rad(u))) / CalcParData)) * Math.sin(deg2rad(v)), 0.5 * (CalcParData - (C / CalcParData)) * Math.sin(deg2rad(u))];
}
